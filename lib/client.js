	...
	factory: (require) => {
		...
		*/
		...
		function isAgentTextData(value) {
			...
			if (value.source !== void 0 && value.source !== "assistant" && value.source !== "plan") return false;
		...
		*/
		function applyActivityRecords(state, records) {
			for (const record of records) {
				...
				if (record.type === "cursor-agent/agent-observed") {
				...
				if (record.type === "cursor-agent/agent-text") {
					const last = state.texts.at(-1);
					if (last !== void 0 && last.epoch === state.epoch && last.kind === record.data.kind && last.source === record.data.source && last.trajectoryId === record.data.trajectoryId && last.parentTrajectoryId === record.data.parentTrajectoryId) {
					...
					state.texts.push({
						...
						...record.data.source === void 0 ? {} : { source: record.data.source },
						text: record.data.text
					});
					continue;
				}
				if (record.type === "cursor-agent/user-question-answer" || record.type === "cursor-agent/full-access-authorized" || record.type === "cursor-agent/request-telemetry" || record.type === "cursor-agent/usage-snapshots") continue;
		...
		*/
		...
		async function loadActivityPage(rpc, sessionId, afterSeq, signal, metrics) {
			...
			if (!result.ok) {
				...
				if (result.error?.code === "stale-cursor") throw new StaleActivityCursorError(message);
		...
		*/
		async function followNativeHistory(sessionId, entry, rpc, controller) {
			let changed = false;
		...
		*/
		function getNativeHistoryStore(rpc, sessionId) {
			...
			return {
				...
				getMetrics: () => copyNativeActivityMetrics(entry.metrics),
				refresh: () => {
					entry.controller?.abort();
					entry.controller = void 0;
					if (entry.timer !== void 0) {
		...
		function isInjectedContext(event) {
			if (event.type !== "user/message") return false;
			const kind = sourceKindOf(event);
			return typeof kind === "string" && kind !== "user";
		}
		function turnOf(event) {
			if (event.type === "user/message") return isInjectedContext(event) ? openNativeTurn : void 0;
		...
		*/
		function visibleNativeTexts(rows, answers) {
			const nonempty = answers.filter((answer) => answer.length > 0).reverse();
			if (nonempty.length === 0) return rows;
			return rows.flatMap((row) => {
				if (row.kind === "thought" || row.parentTrajectoryId !== void 0 || row.source === "plan") return [row];
		...
		*/
		...
		function CursorAgentReadonlyCard({ callId, toolName, nativeName, summary, block, t, conversationT }) {
			...
			const rawTodos = toolName === "todo_write" ? parseRecord(args)?.todos : void 0;
			const todos = Array.isArray(rawTodos) ? rawTodos.map((item) => ({
				...record(item),
				status: String(record(item).status).replace(/^TODO_STATUS_/u, "").toLowerCase()
			})) : void 0;
			if (Array.isArray(todos) && todos.every((item) => typeof record(item).content === "string" && [
				"pending",
				"in_progress",
				"completed"
			].includes(String(record(item).status)))) {
				...
				summary = conversationT("todo.completed", {
					done,
					total: todos.length
				}) + (active === void 0 ? "" : sep + String(record(active).content));
			}
			...
			return /* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
				...
				children: /* @__PURE__ */ (0, react_jsx_runtime.jsx)(_deepseek_ai_dsh_client_ui_primitives.DisclosureRow, {
					...
					children: /* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
						...
						children: /* @__PURE__ */ (0, react_jsx_runtime.jsx)(ToolBody, {
							toolName,
							args,
							result,
							state: rowState,
		...
		function iconOf(toolName) {
			switch (toolName) {
				case "todo_write": return /* @__PURE__ */ (0, react_jsx_runtime.jsx)(_deepseek_ai_dsh_client_ui_primitives.IconChecklistOutline14, { size: 14 });
		...
		function ToolBody({ toolName, args, result, state, t }) {
			...
			const inspectable = (body) => /* @__PURE__ */ (0, react_jsx_runtime.jsxs)(react_jsx_runtime.Fragment, { children: [
				body,
				...
				inspect ? /* @__PURE__ */ (0, react_jsx_runtime.jsx)(IOCard, {
					args,
					result,
					state,
					t
			...
			const text = toolName === "read" && typeof output === "string" && output === result && /^[\s]*[\[{]/u.test(output) ? void 0 : outputText(output);
			if (toolName === "read" && state !== "error" && result === "") return inspectable(/* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
				"data-card-empty-result": true,
				style: {
					color: "var(--dsw-alias-label-tertiary)",
					fontSize: 13
				},
				children: t("terminal.noOutput")
			}));
			...
			if (state !== "error") {
				if (toolName === "read" && path !== void 0 && text !== void 0) {
					...
					return inspectable(/* @__PURE__ */ (0, react_jsx_runtime.jsx)(_deepseek_ai_dsh_client_ui_primitives.ReadBlock, {
						label: path,
						lines,
						totalLines,
						maxLines: 8,
				...
				if (toolName === "edit" || toolName === "write") {
					...
					if (diffs.length > 0) return inspectable(/* @__PURE__ */ (0, react_jsx_runtime.jsx)(_deepseek_ai_dsh_client_ui_primitives.DiffBlock, {
						diffs,
						maxLines: 9,
						labels: {
							...labels("diff"),
				...
				if (toolName === "bash" && typeof command === "string" && (result === void 0 || terminalOutput !== void 0)) {
					...
					return inspectable(/* @__PURE__ */ (0, react_jsx_runtime.jsx)(_deepseek_ai_dsh_client_ui_primitives.TerminalBlock, {
						command,
						output: terminalOutput,
						running: state === "running",
						maxLines: Infinity,
						cwd: typeof input.workdir === "string" ? input.workdir : void 0,
						exitCode: typeof exitCode === "number" && Number.isInteger(exitCode) ? exitCode : void 0,
						labels: {
							...labels("terminal"),
			...
			return /* @__PURE__ */ (0, react_jsx_runtime.jsx)(IOCard, {
				args,
				result,
				state,
				t
		...
		function IOCard({ args, result, state, t }) {
			...
			return /* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
				...
				children: [["row.input", args || void 0], ["row.output", result]].map(([label, text]) => text === void 0 ? null : /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
					"data-card-result": label === "row.output" ? state : void 0,
					style: {
						...ioSection,
						borderTop: label === "row.output" && args !== "" ? "1px solid var(--dsw-alias-border-l2)" : void 0
					},
					children: [
						...
						/* @__PURE__ */ (0, react_jsx_runtime.jsx)("button", {
							...
							onClick: () => {
								(0, _deepseek_ai_dsh_client_ui_primitives.writeClipboard)(pretty(text)).then((ok) => {
  ...
  pointer-events: none;
		...
		*/
		...
		function NativeTurnContainer(props) {
			...
			const branches = (0, react.useMemo)(() => groupNativeActivity(rows, knownIndex < 0 ? [] : rowsForTurnWindow(history.agents, startMs, followingStartMs, Date.now(), includeEarlier), knownIndex < 0 ? [] : visibleNativeTexts(rowsForTurnWindow(history.texts ?? [], startMs, followingStartMs, Date.now(), includeEarlier), answers)), [
				rows,
				history.agents,
				history.texts,
				knownIndex,
