window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const CURSOR_PLUGIN_RPC_ENDPOINT = "plugin-rpc/cursor";
		...
		function callCursorPluginRpc(rpc, endpoint, payload, signal) {
			return rpc.call("/api", CURSOR_PLUGIN_RPC_ENDPOINT, {
				endpoint,
				payload
			}, signal);
		}
		...
		*/
		...
		async function loadActivityPage(rpc, sessionId, afterSeq, signal) {
			const result = await callCursorPluginRpc(rpc, ACTIVITY_READ_AFTER_ENDPOINT, {
				sessionId,
				afterSeq
			}, signal);
			if (!result.ok) {
		...
		*/
		...
		function nativeToolDisplaySummary(state, toolName, t) {
			if (toolName !== "todo_write" || state.input === void 0) return nativeToolSummary(state, toolName);
			...
			const todos = rawTodos.map((item) => {
				...
				return {
					...
					status: String(value.status).replace(/^TODO_STATUS_/u, "").toLowerCase()
				};
			});
			if (!todos.every((item) => typeof item.content === "string" && [
				"pending",
				"in_progress",
				"completed"
			].includes(item.status))) return nativeToolSummary(state, toolName);
			const done = todos.filter((item) => item.status === "completed").length;
			const active = todos.find((item) => item.status === "in_progress");
			return t("todo.completed", {
				done,
				total: todos.length
			}) + (active === void 0 ? "" : " · " + String(active.content));
		}
		...
		function CursorAgentToolNode({ row, conversationT }) {
			...
			return /* @__PURE__ */ (0, react_jsx_runtime.jsx)(NativeToolCard, {
				...model,
				summary: nativeToolDisplaySummary(row.state, model.toolName, translate),
				t: translate
			});
		}
		//#endregion
		...
		function NativeSubagentNode({ branch, ...labels }) {
			...
			return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("section", {
				...
				children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("style", { children: SWEEP }), /* @__PURE__ */ (0, react_jsx_runtime.jsx)(_deepseek_ai_dsh_client_ui_primitives.DisclosureRow, {
					icon: /* @__PURE__ */ (0, react_jsx_runtime.jsx)(_deepseek_ai_dsh_client_ui_primitives.IconAgentPresetOutlineRegular, { size: 14 }),
		...
		*/
		...
		function apply(ctx) {
			...
			installProviderDirectory(ctx, () => acceptedRow?.models.length, {
				...
				binding: {
					channel: CURSOR_PLUGIN_RPC_ENDPOINT,
					endpoint: ACTIVITY_BINDING_ENDPOINT
				}
			});
			const load = async () => {
				const result = await callCursorPluginRpc(rpc, SNAPSHOT_ENDPOINT, {}, void 0);
			...
			const run = async (action, value) => {
				const result = await callCursorPluginRpc(rpc, "run", {
					action,
					...value === void 0 ? {} : { value }
				}, void 0);
				if (!result.ok) throw new Error(result.error.message);
			...
			const pick = async () => {
				const result = await callCursorPluginRpc(rpc, PICK_ENDPOINT, {}, void 0);
				if (!result.ok) throw new Error(result.error.message);
