...
try {
  ...
  const readSelector = await cards.filter({ has: page.locator('[data-read]') }).first().evaluate(node => `[data-native-tool-card="${CSS.escape(node.getAttribute('data-native-tool-card'))}"]`)
  const readCard = page.locator(readSelector)
