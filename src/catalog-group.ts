...
import type { CursorCatalogModel, CursorEffort, CursorModelVariant } from './client-contract.js'

export type CursorModelSelection = { model: string; parameters: Record<string, string> }

export function selectionId(selection: CursorModelSelection): string {
  const pairs = Object.entries(selection.parameters).sort(([a], [b]) => a.localeCompare(b))
  return pairs.length === 0 ? selection.model : selection.model + '[' + pairs.map(([key, value]) => encodeURIComponent(key) + '=' + encodeURIComponent(value)).join(',') + ']'
}

export function parseModelSelection(id: string): CursorModelSelection {
  const start = id.indexOf('[')
  if (start < 0) return { model: id, parameters: {} }
  if (start === 0 || !id.endsWith(']')) throw new Error('Invalid Cursor model selection')
  const parameters: Record<string, string> = Object.create(null)
  for (const pair of id.slice(start + 1, -1).split(',')) {
    const separator = pair.indexOf('=')
    if (separator < 1) throw new Error('Invalid Cursor model parameter')
    const key = decodeURIComponent(pair.slice(0, separator)), value = decodeURIComponent(pair.slice(separator + 1))
    if (!key || !value || Object.hasOwn(parameters, key)) throw new Error('Invalid Cursor model parameter')
    parameters[key] = value
  }
  return { model: id.slice(0, start), parameters }
}

export function parameterKind(id: string): 'context' | 'fast' | 'effort' | undefined {
  if (/context/i.test(id)) return 'context'
  if (/fast/i.test(id)) return 'fast'
  if (/effort|reason|think|thought/i.test(id)) return 'effort'
  return undefined
}

export function contextTokens(value: string | undefined): number | undefined {
  if (value === undefined) return undefined
  const match = /^([1-9][0-9]*)([km])?$/i.exec(value)
  if (match === null) return undefined
  const tokens = Number(match[1]) * (match[2]?.toLowerCase() === 'm' ? 1_000_000 : match[2]?.toLowerCase() === 'k' ? 1000 : 1)
...
/** Cursor-published 200k default (Composer, Claude 4.x, Gemini, Kimi K3, GLM). */
...
/** GPT-5.x default context (Cursor docs: 272k). */
export const CURSOR_GPT_5_CONTEXT_WINDOW = 272_000
/** Kimi K2.7 Code default context (Cursor docs: 262k). */
export const CURSOR_KIMI_K2_CONTEXT_WINDOW = 262_000
...
export function clusterOf(family: string): string {
...
export function sortGroupedFamilies<T extends CursorCatalogModel>(
  grouped: T[],
...
export function familyHasExtendedContext(familyId: string, name = ''): boolean {
  ...
  if (/^claude-sonnet-4\.[5-9]/u.test(id)) return true
  if (/^claude-opus-4\.[6-9]/u.test(id)) return true
  if (id.startsWith('gemini-')) return true
...
/** Cursor-published default for a non-Max family. Undefined when neither the id nor the docs table knows. */
export function defaultContextWindowForFamily(familyId: string): number | undefined {
  const tier = parseCursorContextSuffix(familyId)
  if (tier.tokens !== undefined) return tier.tokens
  const id = clusterOf(familyId).toLowerCase()
  if (id.startsWith('composer')) return CURSOR_DEFAULT_CONTEXT_WINDOW
  if (id.includes('grok')) return CURSOR_GROK_CONTEXT_WINDOW
  if (id.startsWith('gpt-5')) return CURSOR_GPT_5_CONTEXT_WINDOW
  if (id.startsWith('claude-fable-5') || id.startsWith('claude-opus-5') || id.startsWith('claude-sonnet-5')) return CURSOR_CLAUDE_5_CONTEXT_WINDOW
  if (id.startsWith('claude')) return CURSOR_DEFAULT_CONTEXT_WINDOW
  if (id.startsWith('gemini')) return CURSOR_DEFAULT_CONTEXT_WINDOW
  if (/^kimi-k3(?:-|$)/u.test(id)) return CURSOR_DEFAULT_CONTEXT_WINDOW
  if (id.startsWith('kimi-k2')) return CURSOR_KIMI_K2_CONTEXT_WINDOW
  if (id.startsWith('glm-')) return CURSOR_DEFAULT_CONTEXT_WINDOW
}
...
export function groupCursorModels(
  ...
  for (const [family, members] of families) {
    ...
    const row = (id: string, rowName: string, max: boolean): CursorCatalogModel => {
      const contextWindow = max ? CURSOR_MAX_CONTEXT_WINDOW : defaultContextWindowForFamily(id)
      return {
        id,
        name: rowName,
        thinking,
    ...
    // ACP context variants come from advertised parameters, never family-name heuristics.
  }
