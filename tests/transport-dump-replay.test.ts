...
it('does not replay a cancelled turn, a healthy answer, or a non-dump failure', async () => {
  ...
  const dumpShaped = await streamDump([{ result: { status: 'failed', text: CANCEL, error: 'provider turn failed' } }])
  expect(dumpShaped.prompts).toEqual(['Ship the fix.'])
  expect(dumpShaped.finish?.reason.kind).toBe('error')
})
